...
public class ModuleIOSim implements ModuleIO {
   ...
   private static final DCMotor turnMotorModle = DCMotor.getNeoVortex(1);
  private static final double LOOP_PERIOD_SECS = 0.02;
    // change gearbox argument
    private DCMotorSim driveSim = new DCMotorSim(
            LinearSystemId.createDCMotorSystem(driveMotorModel, 0.025, DriveConstants.DriveGearing),
            driveMotorModel); // needs to change
    private DCMotorSim turnSim = new DCMotorSim(
            LinearSystemId.createDCMotorSystem(turnMotorModle, 0.004, DriveConstants.TurnGearing),
            turnMotorModle); // needs to change
